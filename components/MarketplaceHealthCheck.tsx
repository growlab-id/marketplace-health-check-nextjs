...
export default function MarketplaceHealthCheck() {
  ...
  const updateRevenue = (index: number, value: string) => {
    ...
    const newRev = [...data.monthlyRevenue] as [number | null, number | null, number | null];
    newRev[index] = parsed;
  ...
  const updateProduct = (index: number, field: keyof ProductData, value: any) => {
    ...
    if (['revenue', 'price', 'hpp', 'adSpend'].includes(field) && typeof value === 'string') {
    ...
    const newProducts = [...data.topProducts] as [ProductData, ProductData, ProductData];
  ...
  const calculateResults = (formData: FormData) => {
    ...
    const allProductsZero = formData.topProducts.every(p =>
      (!p.revenue || p.revenue === 0) &&
      (!p.price || p.price === 0) &&
      (!p.hpp || p.hpp === 0) &&
      (!p.adSpend || p.adSpend === 0) &&
      (typeof p.roasRoi === 'string' ? (!parseROAS(p.roasRoi) || parseROAS(p.roasRoi) === 0) : (!p.roasRoi || p.roasRoi === 0))
    );

    if (allRevenueZero && allProductsZero) {
      return {
        score: 0,
        details: { trend: 0, concentration: 0, profitability: 0, adPerformance: 0 },
        productResults: formData.topProducts.map(() => ({ isProfitable: false, isAdGood: false, bep: Infinity })),
        ratio: 0
      };
    }

    let score = 0;
    const details = { trend: 0, concentration: 0, profitability: 0, adPerformance: 0 };
    ...
    const top3Total = formData.topProducts.reduce((sum, p) => sum + (p.revenue || 0), 0);
    ...
    const productResults = formData.topProducts.map(p => {
      ...
      const currentRoasRoi = typeof p.roasRoi === 'string' ? (parseROAS(p.roasRoi) || 0) : (p.roasRoi || 0);
  ...
  const saveToSheet = async (sheetName: 'full_submit' | 'partial_submit', score?: number) => {
    let success = false;
    let attempts = 0;
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto relative">
        <header className={`mb-8 text-center transition-all duration-500 ${step === 0 ? 'mt-12 mb-16' : ''}`}>
          <motion.div
          ...
          <motion.div
            ...
            className="mb-8"
          >
            <span className="bg-indigo-600 text-white font-black text-base md:text-lg tracking-[0.5em] uppercase px-10 py-3 rounded-2xl shadow-2xl shadow-indigo-200 inline-block border-b-4 border-indigo-800">
              GROWLAB TOOLS
            </span>
          </motion.div>
          <motion.h1
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  >
                    ...
                    <div className="relative p-8 flex flex-col items-center gap-6 border border-slate-100 rounded-2xl group-hover:border-orange-200 transition-colors">
                      ...
                      <div className="text-center">
                        <h3 className="text-2xl font-black text-slate-900 mb-2">Shopee</h3>
                        <p className="text-slate-700 text-sm font-bold px-4">segera cek kesehatan Shopee Anda sebelum semakin buruk dan sulit recovery</p>
                      </div>
                  ...
                  >
                    ...
                    <div className="relative p-8 flex flex-col items-center gap-6 border border-slate-100 rounded-2xl group-hover:border-slate-300 transition-colors">
                      ...
                      <div className="text-center">
                        <h3 className="text-2xl font-black text-slate-900 mb-2">TikTok Shop</h3>
                        <p className="text-slate-700 text-sm font-bold px-4">segera cek kesehatan Tiktok Shop Anda sebelum semakin buruk dan sulit recovery</p>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 border-t border-slate-200">
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-sm">Hasil Instan</h4>
                      <p className="text-xs text-slate-600 font-bold">Analisis dengan Cepat</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-sm">Metrik Akurat</h4>
                      <p className="text-xs text-slate-600 font-bold">Formula BEP & Profit</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-sm">Trusted Tool</h4>
                      <p className="text-xs text-slate-600 font-bold">Digunakan 1000+ Seller</p>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="space-y-6 mb-8">
                  ...
                  <div className="input-group">
                    <label className="label">Nomor Handphone / WA Aktif Anda</label>
                    <input
                      type="tel"
                      inputMode="numeric"
                      className="input"
                ...
                <div className="flex justify-between">
                  <button
                    onClick={() => {
                      setData(prev => ({ ...prev, userName: '', phoneNumber: '' }));
                      setStep(0);
                    }}
                  ...
                  <button
                    ...
                    disabled={!data.userName || !data.shopName || !data.phoneNumber}
                  >
            ...
            {step === 2 && (
              ...
              >
                <div className="flex items-center gap-2 mb-6 text-indigo-600">
                  <BarChart3 size={24} />
                  <h2 className="text-xl font-bold">GMV Pesanan Siap Dikirim 3 Bulan Terakhir</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  {['3 Bulan Lalu', '2 Bulan Lalu', '1 Bulan Terakhir'].map((label, i) => (
              ...
              >
                <div className="card p-6 md:p-8">
                  <div className="flex items-center gap-2 mb-2 text-indigo-600">
                    <Target size={24} />
                    <h2 className="text-xl font-bold">Top 3 Produk dengan GMV Pesanan Siap Dikirim Tertinggi (1 Bulan Terakhir)</h2>
                  </div>
                  <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-r-lg mb-8">
                    <p className="text-indigo-900 font-bold mb-2">
                      Silakan masukkan data performa produk Anda khusus untuk periode <span className="underline decoration-2 underline-offset-4">1 bulan terakhir</span>.
                    </p>
                    <div className="text-sm text-indigo-700 font-bold leading-relaxed">
                      {data.platform === 'Shopee' ? (
                        <div>
                          <p>💡 Cara cek GMV Pesanan Siap Dikirim setiap produk:</p>
                          <p className="ml-6 text-indigo-800 mt-1">Performa Toko &gt;&gt; Produk &gt;&gt; Performa Produk &gt;&gt; Pilih Periode Data yang Sesuai</p>
                        </div>
                      ) : (
                        <p>💡 Cek GMV Pesanan Siap Dikirim dari setiap produk pada bagian Kompas Data</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    {data.topProducts.map((product, i) => (
                      <div key={i} className="p-6 rounded-xl bg-slate-50 border border-slate-200 space-y-4">
                        ...
                        <div className="bg-indigo-50 border-l-4 border-indigo-500 p-3 rounded-r-lg">
                          <p className="text-sm text-indigo-700 font-bold">
                            Silakan masukkan data performa produk Anda khusus untuk periode 1 bulan terakhir.
                          </p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          <div className="input-group">
                            <label className="label">GMV Pesanan Siap Dikirim</label>
                            <input
                              type="text"
                              className="input pl-4 py-2 text-sm"
                              placeholder="Masukkan nominal GMV"
                              value={formatIDR(product.revenue)}
                              onChange={(e) => updateProduct(i, 'revenue', e.target.value)}
                            />
                          </div>
                          <div className="input-group">
                            <label className="label">Harga Jual (Coret) per unit</label>
                            <input
                              type="text"
                              className="input pl-4 py-2 text-sm"
                          ...
                          <div className="input-group">
                            ...
                            <input
                              ...
                              onChange={(e) => updateProduct(i, 'hpp', e.target.value)}
                            />
                          </div>
                          <div className="input-group">
                          ...
                          <div className="input-group">
                            <label className="label">{data.platform === 'Shopee' ? 'ROAS' : 'ROI'}</label>
                            <input
                              type="text"
                              className="input py-2 text-sm"
                              placeholder={data.platform === 'Shopee' ? 'Contoh 12,5' : 'Contoh 12.5'}
                              value={product.roasRoi ?? ''}
                              onChange={(e) => updateProduct(i, 'roasRoi', e.target.value)}
                            />
                          </div>
                        </div>
                      </div>
            ...
            {step === 4 && results && (
              ...
              >
                <div className="card p-8 text-center bg-gradient-to-br from-indigo-600 to-violet-700 text-white border-none">
                  <h2 className="text-xl font-medium opacity-90 mb-2">Skor Kesehatan Toko Anda</h2>
                  <div className="flex justify-center gap-2 mb-4">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={40}
                        fill={i < Math.round(results.score) ? "currentColor" : "none"}
                        className={i < Math.round(results.score) ? "text-yellow-400" : "text-white/30"}
                      />
                    ))}
                  </div>
                  <div className="text-6xl font-black mb-2">
                    {results.score.toFixed(1)}<span className="text-2xl opacity-50">/5.0</span>
                  </div>
                  <p className="text-lg font-medium text-indigo-100">
                    {results.score >= 4 ? "Sangat Sehat! Pertahankan performa Anda." :
                      results.score >= 3 ? "Cukup Sehat. Ada beberapa area yang perlu ditingkatkan." :
                        results.score >= 2 ? "Kurang Sehat. Segera lakukan optimasi pada toko Anda." :
                ...
                <div className="space-y-6 py-4">
                  <div className="text-center space-y-2">
                    <h3 className="font-bold text-slate-900 text-lg">Butuh Bantuan Lebih Lanjut?</h3>
                    <p className="text-slate-600 font-medium">Konsultasikan hasil Marketplace Health Check Anda dengan tim expert Growlab</p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="card p-6">
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <div className="text-3xl font-bold">
                        {results.details.trend === 1 ? "Naik Stabil" : results.details.trend === 0.5 ? "Fluktuatif" : "Menurun"}
                      </div>
                      <div className="text-sm text-slate-500 mb-1">+{results.details.trend} poin</div>
                    </div>
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                      <div className="bg-indigo-600 h-full transition-all" style={{ width: `${(results.details.trend / 1) * 100}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="card p-6">
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <div className="text-3xl font-bold">{(results.ratio * 100).toFixed(0)}%</div>
                      <div className="text-sm text-slate-500 mb-1">+{results.details.concentration} poin</div>
                    </div>
                    <p className="text-xs text-slate-500 mb-2">Kontribusi Top 3 produk terhadap total omzet</p>
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                      <div className="bg-indigo-600 h-full transition-all" style={{ width: `${(results.details.concentration / 1) * 100}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="card p-6">
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <div className="text-3xl font-bold">{results.productResults.filter(r => r.isProfitable).length}/3</div>
                      <div className="text-sm text-slate-500 mb-1">+{results.details.profitability} poin</div>
                    </div>
                    <p className="text-xs text-slate-500">Produk yang memiliki margin positif setelah biaya platform</p>
                  </div>
                  ...
                  <div className="card p-6">
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <div className="text-3xl font-bold">{results.productResults.filter(r => r.isAdGood).length}/3</div>
                      <div className="text-sm text-slate-500 mb-1">+{results.details.adPerformance} poin</div>
                    </div>
                    <p className="text-xs text-slate-500">Produk dengan {data.platform === 'Shopee' ? 'ROAS' : 'ROI'} di atas BEP</p>
                  </div>
                </div>
                ...
                <div className="card overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 border-bottom border-slate-200">
                      <tr>
                        <th className="p-4 text-sm font-bold text-slate-700">Produk</th>
                        <th className="p-4 text-sm font-bold text-slate-700">Status Profit</th>
                        <th className="p-4 text-sm font-bold text-slate-700">BEP {data.platform === 'Shopee' ? 'ROAS' : 'ROI'}</th>
                        <th className="p-4 text-sm font-bold text-slate-700">Status Iklan</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {data.topProducts.map((p, i) => (
                        <tr key={i}>
                          <td className="p-4 text-sm font-medium">{p.name || `Produk ${i + 1}`}</td>
                          <td className="p-4">
                          ...
                          <td className="p-4 text-sm font-mono">
                            {results.productResults[i].bep === Infinity ? "N/A" : results.productResults[i].bep.toFixed(2)}
                          </td>
                          <td className="p-4">
        ...
        <footer className="mt-12 text-center text-slate-400 text-sm">
          &copy; {new Date().getFullYear()} Marketplace Health Check. Dibuat oleh PT Growlab Digital Solution
        </footer>
      </div>
    </div>
  );
